...
- **[pinescript/kioxia_rebound_trend.pine](pinescript/kioxia_rebound_trend.pine)**：
  ...
  実装。**未解決の論点（共有シートC-075で訂正）**：既存の紙トレード比較は
  entry価格と引け時点などのVWAPの位置関係で分けた代理指標（各群n=21/22）であり、
  エントリー時点のVWAPや押し目の経路を検証していない。2026-09-14の個別事例も
  C-002/C-003で「VWAPを一度割り、再奪還後にエントリー」へ訂正済み。
  正式のREBOUND LONGはVWAP奪還型。`entryMode`はVWAP下エントリー型との
  比較検証用で、どちらかの収益性が確認されたことを意味しない。
