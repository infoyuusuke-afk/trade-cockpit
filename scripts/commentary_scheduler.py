...
def schedule(candidates,last_spoken_at=None,cooldown_seconds=20,max_items=1):
    ...
    if last is not None and last.tzinfo is None: raise ValueError("last_spoken_at must be timezone-aware")
    for c in ordered:
        ts=datetime.fromisoformat(c["timestamp"])
        if ts.tzinfo is None: raise ValueError("candidate timestamp must be timezone-aware")
