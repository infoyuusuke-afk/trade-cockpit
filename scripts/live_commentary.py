...
def _session_safe_text(event,text):
    p=event.get("payload") or {}; ctx=p.get("market_context") or {}
    state=p.get("session_state") or ctx.get("session_state"); quality=p.get("data_quality") or ctx.get("data_quality")
    if state in EXPECTED_INACTIVE_STATES and quality not in BAD_DATA_STATES:
        lowered=str(text).lower()
        if any(x in lowered for x in ("未確認","unconfirmed","missing","データなし","取得不能")): return None
    return text

def commentary_candidate(event):
    p=event.get("payload") or {}; text=p.get("commentary_text") or p.get("summary")
    if not text:return None
    text=_session_safe_text(event,text)
    if text is None:return None
    speech=_session_safe_text(event,p.get("speech_text") or text)
...
def select_commentary(events,limit=3):
    rows=[x for x in (commentary_candidate(e) for e in events) if x]; rows.sort(key=lambda x:(x["priority"],x["timestamp"],x["event_id"]))
    seen=set(); out=[]
    for x in rows:
