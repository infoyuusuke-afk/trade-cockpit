...
def build_queue(items):
    ...
    out=[]; seen=set()
    for i in items:
        if not isinstance(i,dict) or set(i)!=INPUT_FIELDS: raise ValueError("invalid approval item")
        t=i["approval_type"]
        ...
        out.append({"approval_id":aid,"approval_type":t,"created_at":i["created_at"],"summary":i["summary"],"status":"PENDING_OWNER","auto_approve":False})
