...
def load_replay_csv(path,symbol="TSE:285A",timeframe="15S"):
    ...
    for a,b in zip(rows,rows[1:]):
        da=parse_market_ts(a["timestamp"]);db=parse_market_ts(b["timestamp"])
        if db<=da:raise ValueError("NON_INCREASING_TIMESTAMP")
        deltas.append((db-da).total_seconds())
    if str(timeframe).upper()=="15S" and 15.0 not in deltas:
        raise ValueError("NO_15S_CADENCE_EVIDENCE")
    return rows
