...
def main():
    ...
    # 表記の両方が存在するかを別々に検査する形へ変更。
    # 2026-09-19: バージョン番号を"Ver.5.2"固定文字列で検査していたため、Codex側がVer.5.4へ
    # 上げた際にこのチェックだけが取り残されCIがPUBLICATION BLOCKEDになっていた（直前の
    ...
    if not re.search(r"Ver\.\d+\.\d+", html):
        errors.append("index.html missing marker: Ver.X.Y version string")
