...
def project_cycle(events,orchestration,*,last_spoken_at=None,cooldown_seconds=20):
    ...
    return {"council_status":orchestration["council"]["status"],"command_center":orchestration["command_center"],"commentary":spoken,"journal":journal_rows(events),"stories":story_candidates([sanitize_event(e) for e in events]),"owner_approval_required":True,"real_submit_allowed":False,"external_publish_allowed":False}
