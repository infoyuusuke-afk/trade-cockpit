...
def market_session_state(kind: str, now: datetime) -> str:
    ...
    if now.weekday() >= 5: return "CLOSED_KNOWN"
    minute = now.hour * 60 + now.minute
    if kind == "japan":
        if 9*60 <= minute <= 11*60+30 or 12*60+30 <= minute <= 15*60+30: return "OPEN"
        if 11*60+30 < minute < 12*60+30: return "BREAK"
        return "NOT_OPEN_YET" if minute < 9*60 else "CLOSED_KNOWN"
    if kind == "us_close": return "CLOSED_KNOWN"
    if kind == "realtime": return "OPEN"
    return "INVALID"
...
def narration_availability(row: dict) -> str:
    if row.get("verified"): return "AVAILABLE"
    if row.get("session_state") in {"NOT_OPEN_YET","CLOSED_KNOWN","BREAK"}: return "EXPECTED_INACTIVE"
    return "DATA_INVALID"
...
def parse(html: str, now: datetime) -> dict:
    ...
    for code, (label, kind) in INSTRUMENTS.items():
        ...
        rows[code] = {
            ...
            "status": ("表示可" if verified else ("市場未開場・異常ではありません" if session_state == "NOT_OPEN_YET" else "未取得または時刻不整合・売買利用禁止")),
        }
