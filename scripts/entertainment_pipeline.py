...
from scripts.content_sanitizer import validate_sanitized
...
def story_candidates(events,min_score=3):
    out=[]
    for e in events:
        if not validate_sanitized(e): raise ValueError("entertainment requires sanitized event")
        kind=e.get("event_type","")
        score=SCORES.get(kind,0)
        if score<min_score: continue
        out.append({"event_id":e["event_id"],"story_score":score,"title_seed":e.get("summary") or kind,"timestamp":e["timestamp"],"external_publish_allowed":False,"owner_approval_required":True})
