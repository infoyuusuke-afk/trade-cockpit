...
<section id="action-dashboard" class="card wide focus-dashboard">
 ...
 <div class="focus-layout">
  ...
  <div class="focus-chart-wrap">
   <div class="focus-chart-head"><div><b id="focus-chart-name"></b><small id="focus-chart-code"></small></div><div class="focus-tf-group" role="group" aria-label="足の切替"><button class="focus-tf" data-tf="1">5分足</button><button class="focus-tf" data-tf="3">15分足</button><button class="focus-tf" data-tf="6">30分足</button><button class="focus-tf" data-tf="12">60分足</button></div><span id="focus-chart-asof">検証中</span></div>
...
document.addEventListener("DOMContentLoaded",()=>{{
 let rows={payload}, activeIndex=0, tfMultiplier=Number(localStorage.getItem("focusChartTf")||"1")||1, yen=n=>Number(n).toLocaleString("ja-JP",{{maximumFractionDigits:1}})+"円";
 ...
 function aggregateBars(bars,n){{if(n<=1)return bars;const out=[];for(let i=0;i<bars.length;i+=n){{const g=bars.slice(i,i+n);if(!g.length)continue;out.push({{o:g[0].o,c:g[g.length-1].c,h:Math.max(...g.map(v=>v.h)),l:Math.min(...g.map(v=>v.l)),v:g.reduce((s,v)=>s+(v.v||0),0)}});}}return out;}}
 function chart(x){{const a=aggregateBars(x.chart||[],tfMultiplier);if(!a.length)return `<div class="chart-empty">チャートデータ更新待ち</div>`;const W=720,H=390,pl=68,pr=14,p=14;const rawLo=Math.min(...a.map(v=>v.l),x.stop),rawHi=Math.max(...a.map(v=>v.h),x.trigger);const pad=Math.max((rawHi-rawLo)*.06,1);const lo=rawLo-pad,hi=rawHi+pad;const y=v=>p+(hi-v)/(hi-lo||1)*(H-p*2),step=(W-pl-pr)/a.length,bw=Math.max(3,step*.55);let s=`<svg viewBox="0 0 ${{W}} ${{H}}" preserveAspectRatio="none">`;for(let i=0;i<5;i++){{let yy=p+i*(H-p*2)/4,py=hi-(hi-lo)*i/4;s+=`<line class="grid" x1="${{pl}}" y1="${{yy}}" x2="${{W-pr}}" y2="${{yy}}"/><text class="axis-label" x="${{pl-6}}" y="${{yy+4}}">${{Math.round(py).toLocaleString("ja-JP")}}</text>`;}}a.forEach((v,i)=>{{let x0=pl+step*(i+.5),up=v.c>=v.o,cl=up?"c-up":"c-down",yo=y(v.o),yc=y(v.c);s+=`<line class="${{cl}}" x1="${{x0}}" y1="${{y(v.h)}}" x2="${{x0}}" y2="${{y(v.l)}}"/><rect class="${{cl}}" x="${{x0-bw/2}}" y="${{Math.min(yo,yc)}}" width="${{bw}}" height="${{Math.max(1,Math.abs(yo-yc))}}"/>`;}});[[x.trigger,"trigger","発動"],[x.pullback_high,"pullback","押し目"],[x.stop,"stop","撤退"]].forEach(z=>{{if(z[0]==null||!Number.isFinite(z[0])||z[0]<lo||z[0]>hi)return;s+=`<line class="level ${{z[1]}}" x1="${{pl}}" y1="${{y(z[0])}}" x2="${{W-pr}}" y2="${{y(z[0])}}"/><text class="label ${{z[1]}}" x="${{pl+4}}" y="${{y(z[0])-5}}">${{z[2]}} ${{yen(z[0])}}</text>`;}});let offTop=0,offBottom=0;[[x.target1,"利確1"],[x.target2,"利確2"]].forEach(t=>{{const tv=t[0],tl=t[1];if(tv==null||!Number.isFinite(tv))return;if(tv>=lo&&tv<=hi){{s+=`<line class="level target" x1="${{pl}}" y1="${{y(tv)}}" x2="${{W-pr}}" y2="${{y(tv)}}"/><text class="label target" x="${{pl+4}}" y="${{y(tv)-5}}">${{tl}} ${{yen(tv)}}</text>`;}}else if(tv>hi){{s+=`<text class="label target offscale" x="${{W-pr}}" y="${{p+12+offTop*13}}">▲ ${{tl}} ${{yen(tv)}}</text>`;offTop++;}}else{{s+=`<text class="label target offscale" x="${{W-pr}}" y="${{H-p-6-offBottom*13}}">▼ ${{tl}} ${{yen(tv)}}</text>`;offBottom++;}}}});return s+`</svg>`;}}
...
<section id="premarket-gap-ranking" class="card wide">
 ...
 <table><thead><tr><th>順位</th><th>会社名＋コード</th><th>気配値</th><th>前日比（気配）</th><th>成行偏り</th><th>寄り前判定</th></tr></thead><tbody id="premarket-gap-rows"><tr><td colspan="6">Windowsコレクター接続待ち</td></tr></tbody></table>
</section>
"""
