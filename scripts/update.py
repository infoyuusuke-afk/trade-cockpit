...
<tr><td>政府資料に納入期待</td><td>IHI（7013）／日本製鋼所（5631）</td><td>経産省が当該SMR案件で関連機器等の納入を期待する企業として明記。契約・受注は未確定</td><td>個社の二経路照合待ち</td><td>個別確認待ち</td><td>投資実施決定・個別受注IR・信用需給を確認</td></tr>
<tr><td>別案件・研究</td><td>三菱重工業（7011）</td><td>上記GE Vernova Hitachi案件の納入期待企業として経産省資料には記載なし</td><td>この枠では非表示</td><td>未評価</td><td>別途個社IRで対米案件との関係を確認</td></tr>
</tbody></table>
...
<section id="watchlist-100" class="card wide">
 ...
 <div id="watchlist-100-rows" class="cc-watch-list"><div class="cc-onair-empty">Windowsコレクター接続待ち</div></div>
</section>
<section id="premarket-gap-ranking" class="card wide">
 <h2>寄り前気配ランキング（100銘柄・リアルタイムLIVE）</h2>
 <p class="sub">8:00〜9:00の気配値変化率（前日終値比）が大きい順。MS2 RSSが100銘柄分リアルタイムで取得した気配データで、Yahoo!ファイナンス等の遅延データではありません。ただし対象は固定100銘柄ウォッチリスト内のみ（このリスト外の銘柄は「外部ランキング発掘」セクションで別途確認してください）。</p>
 <div id="premarket-gap-rows" class="cc-watch-list"><div class="cc-onair-empty">Windowsコレクター接続待ち</div></div>
</section>
"""
...
document.addEventListener("DOMContentLoaded",()=>{
 ...
 async function loadMs2Live(){
  ...
  try{
   ...
   const ms2Direction=x=>x.common_decision==="TREND LONG"?"long":x.common_decision==="TREND SHORT"?"short":x.signal?.includes("回避")?"block":"wait";
   const ms2Metrics=x=>{
    const cr=x.credit_ratio==null?null:`${x.credit_ratio}倍`;
    const shortable=x.shortable_quantity==null?null:Number(x.shortable_quantity).toLocaleString("ja-JP");
    const or5=(x.or5_low==null&&x.or5_high==null)?null:`${yen(x.or5_low)}–${yen(x.or5_high)}`;
    const or15=(x.or_low==null&&x.or_high==null)?null:`${yen(x.or_low)}–${yen(x.or_high)}`;
    const ema=(x.ema9==null&&x.ema20==null)?null:`${yen(x.ema9)} / ${yen(x.ema20)}`;
    return [
     {label:"統合スコア",value:x.common_score==null?null:x.common_score},
     {label:"既存戦略",value:x.strategy},
     {label:"地合い",value:x.market_state},
     {label:"VWAP",value:x.vwap==null?null:yen(x.vwap)},
     {label:"EMA9/20",value:ema},
     {label:"出来高加速",value:x.volume_burst==null?null:`${x.volume_burst}倍`},
     {label:"歩み値",value:x.flow_bias==null?null:`${x.flow_bias}%`},
     {label:"UNDER",value:x.under_ratio==null?null:`${x.under_ratio}%`},
     {label:"信用倍率",value:cr},
     {label:"売建可能数量",value:shortable},
     {label:"OR5",value:or5},
     {label:"OR15",value:or15},
    ];
   };
   cards.innerHTML=xs.length?xs.slice(0,5).map(x=>window.renderCockpitCard({
    symbol:String(x.ticker||x.code||"").replace(".T",""),
    company:x.name,
    direction:ms2Direction(x),
    directionLabel:esc(x.common_decision||"NO TRADE"),
    price:x.price,
    changePct:x.change_pct,
    ageSeconds:age,
    staleThresholdSeconds:60,
    failClosed:stale?"60秒以上前のデータ・売買禁止":false,
    entry:x.entry_price,stop:x.stop_price,target:x.target1,
    metrics:ms2Metrics(x),
    foot:`1分足・5分足を全対象へ共通生成／未取得は未確認／${x.sector||""}`,
   })).join(""):`<div class="focus-empty"><b>発動なし</b><span>条件一致銘柄を待っています</span></div>`;
   ...
   if(holdCards)holdCards.innerHTML=holds.length?holds.slice(0,5).map(x=>{
    ...
    return window.renderScalpCard({name:x.name,ticker:x.ticker,_stale:stale,price:x.reference_price_1525??x.price,vwap:x.vwap,or_low:x.or_low,or_high:x.or_high,ema9:fs.ema9,ema20:fs.ema20,flow_bias:x.flow_bias,foot},sv,"日足");
 ...
 document.addEventListener("ms2RssUpdate",e=>{const d=e.detail||{},box=document.getElementById("watchlist-100-rows");if(!box)return;const all=Array.isArray(d.all_targets)?d.all_targets:[],topTickers=new Set((Array.isArray(d.top5)?d.top5:[]).map(x=>x.ticker)),rest=all.filter(x=>!topTickers.has(x.ticker)).slice().sort((a,b)=>(Number(b.volume_burst)||0)-(Number(a.volume_burst)||0));box.innerHTML=rest.length?rest.slice(0,50).map((x,i)=>{const bidQty=x.bid_qty==null?null:Number(x.bid_qty).toLocaleString("ja-JP"),askQty=x.ask_qty==null?null:Number(x.ask_qty).toLocaleString("ja-JP"),mBuy=x.market_buy==null?null:Number(x.market_buy).toLocaleString("ja-JP"),mSell=x.market_sell==null?null:Number(x.market_sell).toLocaleString("ja-JP");return window.renderCockpitWatchRow({symbol:String(x.ticker||"").replace(".T",""),company:x.name,direction:"wait",showBadge:false,rank:i+1,price:x.price,changePct:x.change_pct,
     sparkline:x.bars_1m,sparklineLabel:"直近15分・1分足終値",freshness:d.stale?"stale":"fresh",freshnessLabel:d.stale?"データ停止":"LIVE",metrics:[{label:"出来高加速",value:x.volume_burst==null?null:Number(x.volume_burst).toFixed(2)+"倍"},{label:"気配 買/売",value:(bidQty==null&&askQty==null)?null:`${bidQty??"—"} / ${askQty??"—"}`},{label:"成行 買/売",value:(mBuy==null&&mSell==null)?null:`${mBuy??"—"} / ${mSell??"—"}`},{label:"UNDER",value:x.under_ratio==null?null:Number(x.under_ratio).toFixed(1)+"%"}]});}).join(""):"<div class=\"cc-onair-empty\">データ取得待ち</div>";});
 document.addEventListener("ms2RssUpdate",e=>{const d=e.detail||{},box=document.getElementById("premarket-gap-rows");if(!box)return;const all=Array.isArray(d.all_targets)?d.all_targets:[],withGap=all.filter(x=>x.preopen_gap_pct!=null).slice().sort((a,b)=>Math.abs(Number(b.preopen_gap_pct)||0)-Math.abs(Number(a.preopen_gap_pct)||0));box.innerHTML=withGap.length?withGap.slice(0,30).map((x,i)=>{const imbalance=x.preopen_market_imbalance==null?null:(Number(x.preopen_market_imbalance)>=0?"+":"")+Number(x.preopen_market_imbalance).toFixed(1)+"%";return window.renderCockpitWatchRow({symbol:String(x.ticker||"").replace(".T",""),company:x.name,direction:"wait",showBadge:false,rank:i+1,price:x.preopen_quote,changePct:x.preopen_gap_pct,freshness:d.stale?"stale":"fresh",freshnessLabel:d.stale?"データ停止":"LIVE",metrics:[{label:"成行偏り",value:imbalance},{label:"寄り前判定",value:x.preopen_plan||"判定待ち"}]});}).join(""):"<div class=\"cc-onair-empty\">寄り前気配データ待ち（8:00〜9:00のみ取得）</div>";});
...
"""
    ...
    phase = data["phase"]
    html = f"""<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="refresh" content="900"><title>AIトレードコクピット</title><script src="https://cdn.jsdelivr.net/npm/lightweight-charts@5.2.1/dist/lightweight-charts.standalone.production.js"></script>
...
<div id="kio-live-monitor" class="kio-live-monitor" aria-live="polite">
 ...
 <div class="kio-monitor-grid">
  ...
  <div><span>5分足 EMA20／VWAP</span><b id="kio-live-ma">—</b><small>表示線を実戦用に限定</small></div>
 </div>
...
<div class="kio-summary">
 ...
 <div class="kio-metric"><span>過去20回・方向一致</span><b id="kio-audit-hit">蓄積中</b></div>
 <div class="kio-metric"><span>予測平均誤差</span><b id="kio-audit-error">蓄積中</b></div>
</div>
...
const timedPath = (points, stroke="#58d9b4", turns=[]) => {{
  if (!points || points.length < 2) return "<div class='focus-empty'>データなし</div>";
  const lo=Math.min(...points),hi=Math.max(...points),span=Math.max(hi-lo,.01);
...
Promise.all([
 ...
 fetch("kioxia_breaking_radar.json?t=" + Date.now()).then(r => r.json()).catch(() => ({{status:"UNAVAILABLE",events:[],source_health:[]}}))
  ...
  document.getElementById("kio-audit-hit").textContent=auditSummary.direction_hit_rate==null?"蓄積中":Number(auditSummary.direction_hit_rate).toFixed(1)+"%（n="+auditSummary.sample+"）";
  document.getElementById("kio-audit-error").textContent=auditSummary.mean_absolute_error==null?"蓄積中":Number(auditSummary.mean_absolute_error).toFixed(2)+"%pt";
  ...
  document.getElementById("kio-gap-current").textContent=d.current_gap_pct==null?"寄り前：気配は統計帯に採用せず、寄り値確定後に照合":"本日 "+signedPct(d.current_gap_pct,2)+"／"+(gaps.find(x=>x.bucket===((d.current_gap_pct<=-5)?"GD 5%以上":d.current_gap_pct<=-3?"GD 3–5%":d.current_gap_pct<=-1?"GD 1–3%":d.current_gap_pct<1?"±1%以内":d.current_gap_pct<3?"GU 1–3%":d.current_gap_pct<5?"GU 3–5%":"GU 5%以上"))?.bucket||"該当帯なし");
...
document.addEventListener("liveFocusUpdate",e=>{{
  ...
  document.getElementById("kio-live-ma").textContent=[k.ema20,k.vwap].map(v=>v==null?"—":Number(v).toLocaleString("ja-JP",{{maximumFractionDigits:1}})).join("／");
