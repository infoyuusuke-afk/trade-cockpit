...
document.addEventListener("DOMContentLoaded",()=>{
 ...
 async function loadMs2Live(){
  ...
  try{
   ...
   const canonicalLive=x=>{if(!x||!x.ticker)return null;const p=(x.live_quote_valid===true)?Number(x.live_price):(x.live_quote_valid==null?Number(x.price):NaN);return Number.isFinite(p)?{...x,price:p,live_price:p}:null};
   ...
   if(holdCards)holdCards.innerHTML=holds.length?holds.slice(0,5).map(x=>{
    ...
    const live=liveByTicker.get(String(x.ticker||""));const displayPrice=d.hold_finalized?(x.reference_price_1525??x.price):(live?live.price:null);const priceLabel=d.hold_finalized?"15:25参照値":"現在値";return window.renderScalpCard({name:x.name,ticker:x.ticker,price:displayPrice,vwap:live?live.vwap:x.vwap,or_low:live?live.or_low:x.or_low,or_high:live?live.or_high:x.or_high,ema9:live?live.ema9:fs.ema9,ema20:live?live.ema20:fs.ema20,flow_bias:live?live.flow_bias:x.flow_bias,foot:priceLabel+"／"+foot+(displayPrice==null?"／LIVE DATA INVALID":"")},displayPrice==null?{label:"INVALID",cls:"block"}:sv,d.hold_finalized?"15:25":"1m");
