...
document.addEventListener("DOMContentLoaded",()=>{
 ...
 async function loadMs2Live(){
  ...
  try{
   ...
   cards.innerHTML=xs.length?xs.slice(0,5).map((x,i)=>{const cls=x.common_decision==="TREND LONG"?"buy":x.common_decision==="TREND SHORT"?"sell":x.signal?.includes("回避")?"block":"watch",cr=x.credit_ratio==null?"未確認":`${esc(x.credit_ratio)}倍`,shortable=x.shortable_quantity==null?"未確認":Number(x.shortable_quantity).toLocaleString("ja-JP"),order=x.entry_price==null?"条件未完成":`発動 ${yen(x.entry_price)} / 損切 ${yen(x.stop_price)} / 1R ${yen(x.target1)}`;return `<article class="ms2-live-card ${cls}"><div class="top"><span class="signal">${stale?"NO TRADE":esc(x.common_decision||"NO TRADE")}</span><h3>#${i+1} ${esc(x.name)}</h3><b class="score">${x.common_score==null?"未確認":esc(x.common_score)}</b></div><div class="live-price">${yen(x.price)}</div><div class="ms2-metrics"><span>既存戦略<b>${esc(x.strategy)}</b></span><span>注文目安<b>${order}</b></span><span>地合い<b>${esc(x.market_state)}</b></span><span>VWAP<b>${yen(x.vwap)}</b></span><span>EMA9/20<b>${yen(x.ema9)} / ${yen(x.ema20)}</b></span><span>出来高加速<b>${esc(x.volume_burst)}倍</b></span><span>歩み値<b>${esc(x.flow_bias)}%</b></span><span>UNDER<b>${esc(x.under_ratio)}%</b></span><span>信用倍率<b>${cr}</b></span><span>売建可能数量<b>${shortable}</b></span><span>OR5<b>${yen(x.or5_low)}–${yen(x.or5_high)}</b></span><span>OR15<b>${yen(x.or_low)}–${yen(x.or_high)}</b></span></div><small>1分足・5分足を全対象へ共通生成／未取得は未確認／${esc(x.sector)}</small></article>`}).join(""):`<div class="focus-empty"><b>発動なし</b><span>条件一致銘柄を待っています</span></div>`;
   ...
   if(holdCards)holdCards.innerHTML=holds.length?holds.slice(0,5).map((x,i)=>`<article class="ms2-live-card ${String(x.hold_signal).includes("ロング")?"buy":"sell"}"><div class="top"><span class="signal">${stale?"無効":esc(x.hold_signal)}</span><h3>#${i+1} ${esc(x.name)}（${esc(x.ticker)}）</h3><b class="score">${esc(x.hold_score)}</b></div><div class="live-price">${yen(x.reference_price_1525??x.price)}</div><div class="ms2-metrics"><span>後場OR上維持<b>${esc(x.pm_above_minutes)}分</b></span><span>後場OR下維持<b>${esc(x.pm_below_minutes)}分</b></span><span>引け位置<b>${esc(x.close_location_pct)}%</b></span><span>地合い<b>${esc(x.market_state)}</b></span></div><small>${d.hold_finalized?"15:25候補固定／15:30終値を取得後、翌取引日引けで採点":"暫定候補／15:25まで注文しない"}</small></article>`).join(""):`<div class="focus-empty"><span>${d.hold_finalized?"15:25確定候補なし・持ち越し禁止":"15:25確定待ち、または条件未達"}</span></div>`;
   ...
   if(holdHistory)holdHistory.innerHTML=recent.length?recent.slice(0,5).map(x=>`<article class="ms2-live-card ${x.result==="勝ち"?"buy":x.result==="負け"?"sell":"watch"}"><div class="top"><span class="signal">${esc(x.result)}</span><h3>${esc(x.name)}（${esc(x.ticker)}）</h3><b class="score">${Number(x.return_close_pct)>=0?"+":""}${esc(x.return_close_pct)}%</b></div><small>${esc(x.decision_date)} ${esc(x.side)}／翌日 ${esc(x.evaluation_date)}／MFE ${esc(x.mfe_pct)}%・MAE ${esc(x.mae_pct)}%</small></article>`).join(""):"";
...
.fp-price-row{{display:flex;align-items:baseline;gap:9px}}.fp-price{{font-size:21px;font-weight:700;letter-spacing:-.01em;font-variant-numeric:tabular-nums;color:#f4f7fa}}.fp-change{{font-size:13px;font-weight:700;font-variant-numeric:tabular-nums;padding:1px 7px;border-radius:5px}}.fp-change.up{{color:#3ed5ae;background:#3ed5ae1a}}.fp-change.down{{color:#ef646b;background:#ef646b1a}}
...
<link rel="stylesheet" href="theme.css?v=20260915-type2">
<link rel="stylesheet" href="focus.css?v=20260915-power2">
