 ...
 async function loadMs2Live(){
  ...
  try{
   ...
   if(holdCards)holdCards.innerHTML=holds.length?holds.slice(0,5).map(x=>{
    ...
    const live=liveByTicker.get(String(x.ticker||""));const ref1525=Number(x.reference_price_1525);const displayPrice=d.hold_finalized?(Number.isFinite(ref1525)&&ref1525>0?ref1525:null):(live?live.price:null);const priceLabel=d.hold_finalized?"15:25固定参照値":"MS2 LIVE現在値";return window.renderScalpCard({name:x.name,ticker:x.ticker,price:displayPrice,vwap:live?live.vwap:null,or_low:live?live.or_low:null,or_high:live?live.or_high:null,ema9:live?live.ema9:null,ema20:live?live.ema20:null,flow_bias:live?live.flow_bias:null,foot:priceLabel+"／"+foot+(displayPrice==null?"／LIVE DATA INVALID":"")},displayPrice==null?{label:"INVALID",cls:"block"}:sv,d.hold_finalized?"15:25":"1m");
